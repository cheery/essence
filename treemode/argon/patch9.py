...
class Patch9(object):
    ...
    def cell(self, (left, top, width, height), (c0,c1,c2,c3), x, y):
        ...
        k0 = argon.mix(c0, c1, float(x0)/width)
        k1 = argon.mix(c0, c1, float(x1)/width)
        k2 = argon.mix(c2, c3, float(x0)/width)
        k3 = argon.mix(c2, c3, float(x1)/width)
        c0 = argon.mix(k0, k2, float(y0)/height)
        c1 = argon.mix(k1, k3, float(y0)/height)
        c2 = argon.mix(k0, k2, float(y1)/height)
        c3 = argon.mix(k1, k3, float(y1)/height)
        return (left+x0, top+y0, x1-x0, y1-y0), (s0, t0, s1, t1), (c0,c1,c2,c3)
